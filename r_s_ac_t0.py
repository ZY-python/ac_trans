...
labeled_idx_0 = random.sample(range(0, 90), 1)

# 此处的labeled_idx_0为后面得到rep做准备
labeled_id_ini = labeled_idx_0

target_loader_i = get_target_l(target_dataset_unl, args, labeled_id_ini)
target_loader_unl_i = get_target_un(target_dataset_unl, args, labeled_id_ini)


for n in range(54):
    all_target = get_data(n, target_loader_i, target_loader_unl_i)
    _, rep = active_L_c(args, len_class_list, model1, labeled_idx_0, all_target, 1)
    if n == 0:
        rep_f = rep
    else:
        rep_f = torch.cat((rep_f, rep), 0)
###########################################目的是得到总的rep特征######################################

# 利用总的特征rep来进行主动学习
labeled_idx = active_f(rep_f, labeled_idx, 126, len_class_list)

result_path = os.path.join('./initial_d_r_s.pkl')
with open(result_path, 'wb') as f0:
    pickle.dump(labeled_idx, f0)
dirname = './data/multi/sketch'
fpath = os.path.join(dirname, 'new_l_u1.txt')
